...
function Lobby(room) {
  ...
  // Rendu initial — structure fixe qu'on met à jour sans recréer
  R(`<div class="sc"><div class="float" style="text-align:center"><div style="font-size:2.5rem">📺</div><h2 style="font-family:'Playfair Display',serif;font-size:1.6rem;margin-top:4px">Écran principal</h2><p style="color:rgba(255,255,255,.38);font-size:.76rem;margin-top:3px">Cet écran est le plateau du jeu</p></div><div class="glass" style="padding:17px 19px;text-align:center;max-width:370px;width:100%"><p style="color:rgba(255,255,255,.42);font-size:.76rem;margin-bottom:6px">Code de la salle</p><div style="font-family:'Orbitron',sans-serif;font-size:2.4rem;font-weight:900;color:${t.accent};animation:roomGlow 2s ease-in-out infinite">${room.code}</div><p style="color:rgba(255,255,255,.28);font-size:.68rem;margin-top:5px;margin-bottom:13px">${(room.themes||[room.theme]).map(tid=>(THEMES[tid]||THEMES.culture).emoji).join(" ")} · ${room.rounds.map(r=>RT.find(x=>x.id===r)?.icon||"").join(" ")}</p><div style="background:rgba(0,0,0,.25);border-radius:11px;padding:11px 13px;text-align:left;border:1px solid rgba(255,255,255,.09);margin-bottom:9px;font-size:.78rem;line-height:1.75;white-space:pre-wrap;user-select:all">${inv}</div><button class="btn" id="bCp" style="width:100%;padding:11px;background:linear-gradient(135deg,${t.dark},${t.accent});color:white;border-radius:13px;font-size:.85rem">📋 Copier le lien</button></div><div class="glass" style="padding:14px 16px;max-width:370px;width:100%"><p style="color:rgba(255,255,255,.48);font-size:.75rem;font-weight:600;margin-bottom:8px">JOUEURS (<span id="jCount">0</span>/${room.maxP})</p><div id="jList"><div style="padding:7px 11px;border-radius:10px;border:1px dashed rgba(255,255,255,.11);color:rgba(255,255,255,.2);font-size:.76rem;text-align:center">En attente de joueurs…</div></div></div><div style="display:flex;gap:10px;width:100%;max-width:370px"><button class="btn" id="bCn" style="background:rgba(255,255,255,.07);color:white;padding:12px;flex:1;font-size:.84rem">✕ Annuler</button><button class="btn" id="bLn" style="padding:12px;flex:2;font-size:.88rem;color:white;background:rgba(255,255,255,.1)" disabled>Attendez des joueurs</button></div></div>`);

  // Attacher les boutons UNE SEULE FOIS
  on("bCn","click",async()=>{ lobbyActive=false; if(STOP)STOP(); await fd(`rooms/${room.code}`); CD={name:"",maxP:4,themes:[],rounds:[],elimR:2,cartonR:3}; Home(); });
  on("bLn","click",doLaunch);
  on("bCp","click",()=>{ navigator.clipboard.writeText(inv).then(()=>{set("bCp","✅ Copié !");setTimeout(()=>set("bCp","📋 Copier le lien"),2500);}).catch(()=>{}); });

  // Mettre à jour seulement la liste des joueurs sans recréer tout le DOM
  function updatePlayers(players) {
    const arr = toArr(players);
    const jList = $("jList");
    const jCount = $("jCount");
    const bLn = $("bLn");
    if (!jList || !jCount || !bLn) return;
    jCount.textContent = arr.length;
    if (arr.length === 0) {
      jList.innerHTML = `<div style="padding:7px 11px;border-radius:10px;border:1px dashed rgba(255,255,255,.11);color:rgba(255,255,255,.2);font-size:.76rem;text-align:center">En attente de joueurs…</div>`;
      bLn.disabled = true;
      bLn.style.background = "rgba(255,255,255,.1)";
      bLn.textContent = "Attendez des joueurs";
    } else {
      jList.innerHTML = arr.map((p,i)=>`<div style="display:flex;align-items:center;gap:9px;padding:7px 11px;border-radius:10px;background:rgba(255,255,255,.04)"><div style="width:26px;height:26px;border-radius:50%;background:${COL[i%8].bg};display:flex;align-items:center;justify-content:center;font-size:.72rem;font-weight:700">${i+1}</div><span style="font-weight:600;font-size:.88rem">${p.name}</span></div>`).join("");
      bLn.disabled = false;
      bLn.style.background = `linear-gradient(135deg,${t.dark},${t.accent})`;
      bLn.textContent = "🚀 Lancer !";
    }
  }
  ...
  STOP = () => { lobbyActive = false; };
  const pollLobby = async () => {
    if (!lobbyActive) return;
    const cur = await fg(`rooms/${room.code}`);
    if (cur && lobbyActive) updatePlayers(cur.players);
    if (lobbyActive) setTimeout(pollLobby, 2000);
  };
