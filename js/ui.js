...
function Create(step) {
  ...
  if (step === 1) {
    R(`<div class="sc"><div class="glass su" style="width:100%;max-width:380px;padding:24px 20px"><button id="bBk" style="background:none;border:none;color:rgba(255,255,255,.4);cursor:pointer;font-size:.8rem;margin-bottom:16px">← Retour</button><h2 style="font-family:'Playfair Display',serif;font-size:1.5rem;margin-bottom:3px">Créer une partie</h2><p style="color:rgba(255,255,255,.38);margin-bottom:18px;font-size:.8rem">Étape 1/3</p><label style="color:rgba(255,255,255,.5);font-size:.75rem;font-weight:600;margin-bottom:5px;display:block">Votre prénom</label><input id="iN" placeholder="Votre prénom" maxlength="14" value="${CD.name}" style="margin-bottom:14px"/><label style="color:rgba(255,255,255,.5);font-size:.75rem;font-weight:600;margin-bottom:7px;display:block">Nombre max de joueurs</label><div id="mpGrid" style="display:flex;gap:7px;flex-wrap:wrap;margin-bottom:20px"></div><button class="btn" id="bNx" style="width:100%;padding:13px;background:linear-gradient(135deg,#7c3aed,#a78bfa);color:white">Suivant →</button></div></div>`);
    function renderMpGrid() {
      ...
      grid.querySelectorAll(".mpBtn").forEach(b => b.addEventListener("click",()=>{ CD.name=val("iN")||CD.name; CD.maxP=+b.dataset.n; renderMpGrid(); }));
    }
    renderMpGrid();
    on("bBk","click",()=>{ CD={name:"",maxP:4,themes:[],rounds:[],elimR:2,cartonR:3}; Home(); });
    on("bNx","click",()=>{ const n=val("iN").trim(); if(!n){alert("Entrez votre prénom");return;} CD.name=n; Create(2); });
...
async function doCreate() {
  ...
  const code   = genCode();
  const room   = { code,phase:"lobby",theme:themes[0],themes,rounds:CD.rounds,elimR:CD.elimR,cartonR:CD.cartonR,maxP:CD.maxP,players:[{name:CD.name,isHost:true}],ts:Date.now() };
  if (!await fs(`rooms/${code}`, room)) { alert("Erreur Firebase."); return; }
  ME=CD.name; CODE=code; HOST=true;
  Lobby(room);
}
...
function Lobby(room) {
  ...
  function draw(cur) {
    ...
    on("bCp","click",()=>{ navigator.clipboard.writeText(inv).then(()=>{set("bCp","✅ Copié !");setTimeout(()=>set("bCp","📋 Copier le message d'invitation"),2500);}).catch(()=>{}); });
  }
  draw(room);
  if (STOP) STOP();
